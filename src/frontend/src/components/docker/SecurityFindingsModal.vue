  ...
  <BaseModal :is-open="isOpen" max-width="640px" @close="$emit('close')">
    <div class="px-6 py-4 border-b border-border">
      ...
      <p class="text-sm text-text-secondary mt-1">
        ...
        Nothing here is changed for you — each finding says what to change in
        Unraid's own Docker edit form.
      </p>
    </div>

    <div class="max-h-[420px] overflow-auto px-6 py-4">
      <p v-if="rows.length === 0" class="text-sm text-text-secondary">
        {{ emptyText }}
      </p>
...
const rows = computed<Row[]>(() => {
  const pick = (c: Container) =>
    showDismissed.value ? securityStore.dismissedFindings(c) : securityStore.findings(c);
